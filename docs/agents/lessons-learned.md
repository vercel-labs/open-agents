...
- SWR client fetches to app `/api` routes should use `cache: "no-store"` (and let SWR handle caching), or browser HTTP cache can replay stale chat-list responses and overwrite fresher server-rendered sidebar state during hydration/polling.
