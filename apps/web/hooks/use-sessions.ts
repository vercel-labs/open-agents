...
export type SessionWithUnread = Session & { hasUnread: boolean; hasStreaming: boolean };
...
export function useSessions(options?: { enabled?: boolean }) {
  ...
  const createSession = async (input: CreateSessionInput) => {
    ...
    await mutate(
      {
        sessions: [{ ...createdSession, hasUnread: false, hasStreaming: false }, ...sessions],
      },
      { revalidate: false },
    );
      ...
      await mutate(
        (current) => ({
          sessions: (current?.sessions ?? []).map((s) =>
            s.id === sessionId
              ? { ...updatedSession, hasUnread: s.hasUnread, hasStreaming: s.hasStreaming }
              : s,
          ),
        }),
        { revalidate: false },
