...
- When refactoring shell command assembly in agent tools, remove superseded flags from the shared args array instead of only overriding them in the wrapper command. In April 2026 `web_fetch` accidentally kept an old process-substitution `-o >(head -c ... >&3)` in `fetch.ts` after switching to file-backed body capture, which produced an invalid curl invocation once the wrapper also added `-o "$body_file"`.
