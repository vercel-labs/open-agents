 ...
 *
 * The prefix `/codespace-proxy/{sessionId}` is **stripped** before rewriting,
 * so code-server sees requests at its root — the same pattern as Caddy's
 * `uri strip_prefix`.  Vercel's edge handles the full connection including
 * WebSocket upgrades.
 */
export async function middleware(request: NextRequest) {
  ...
  // Strip the /codespace-proxy/{sessionId} prefix so code-server sees
  // requests at its root — same as Caddy `uri strip_prefix`.
  const strippedPath =
    slashIndex === -1 ? "/" : `/${afterPrefix.slice(slashIndex + 1)}`;
  const targetUrl = new URL(strippedPath + request.nextUrl.search, sandboxUrl);
