 ...
 * Set VERCEL_SANDBOX_BASE_SNAPSHOT_ID to enable faster startup from a prepared image.
 * When unset, sandboxes are created from the runtime default image.
 */
const configuredBaseSnapshotId =
  process.env.VERCEL_SANDBOX_BASE_SNAPSHOT_ID?.trim();
export const DEFAULT_SANDBOX_BASE_SNAPSHOT_ID =
  configuredBaseSnapshotId && configuredBaseSnapshotId.length > 0
    ? configuredBaseSnapshotId
    : undefined;
