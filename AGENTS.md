...
- GitHub App install flow: use a single unified flow where OAuth authorization happens during installation (enable "Request user authorization (OAuth) during installation" in the GitHub App settings). Do not use a separate OAuth-then-install two-step flow. The app callback (`/api/github/app/callback`) handles both the `code` (OAuth) and `installation_id` params in one redirect.
