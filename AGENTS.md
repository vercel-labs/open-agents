...
- For workflow-managed sandbox lifecycle, avoid client-side timeout auto-stop logic in the chat UI; it can race with workflow rollover/hibernate and produce confusing paused overlays while the tab remains open.
...
- During workflow-driven hard-timeout rollover, the client can briefly observe `no_sandbox` between old sandbox stop and new sandbox connect; keep a short reconnect-retry "rolling over" UI state to avoid flashing misleading `No sandbox`.
...
- If sandbox validity uses a grace window (for example `expiresAt - 10s`), treat that window as a transition in UI (`Rolling over`) rather than `Paused`; otherwise active rollover can be mislabeled as paused.
- Distinguish inactivity hibernation from hard-timeout rollover in the UI state machine: reconnect checks with an existing `snapshotUrl` should render as `Hibernating`, while `Rolling over` should be reserved for near-expiry/hard-timeout transitions.
- In rollover retry logic, a reconnect result of `no_sandbox` should terminate rollover mode for hibernation paths (do not keep labeling `Rolling over` while server has already hibernated).
- Avoid inferring `Rolling over` from heuristic near-expiry timers alone; only show it from explicit rollover transition state, otherwise inactivity hibernation can be mislabeled as rollover.
